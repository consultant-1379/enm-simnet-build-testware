...
public class Read_from_file {
	...
	public int read_Original_MOs(String generated_Kertyle_File_Name)
	{
		...
		try{
	        ...
	        System.out.println("Error while reading file line by line:Read_Present_No_of_MOs"+ e.getMessage()); 
		   }
...
//	System.out.println("size ="+a1.size());
	int i;
	   for(i=0;i<(a1.size()-1);i++) 
		{
			...
			if(a1.get(i).contains("e: csmim:get_mim_file_name_by_nename(\""+NE_Name+"\")."))
			{
				break;
			}
		}
	   ...
	   //{ok, MimFile}
		...
		{
		mim_file_name=mim_file_line.replace("{ok,", "").replace("}","").replace("\"",""); // extracting mimfilename
     ...
     System.out.println("Error while reading file line by line:read_mim_name" 
     + e.getMessage()); 
	   }
	...
	public boolean check_word_in_file(String File_Name, String statement)
	{
		...
		try{
	        ...
	        System.out.println("Error while reading file line by line:Read_Logs"+ e.getMessage()); 
		   }
		return status;
	}
	
	public ArrayList<String> write_file_to_arrlist(String File_to_be_read) throws IOException
	{
	...
	//	String last_line=null;
		try{
	        ...
	        System.out.println("Error while reading file line by line:read_MOs_from_file function "+ e.getMessage()); 
		   }
	return Type;
	}
