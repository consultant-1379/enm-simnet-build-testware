...
public class start_Create_Simulation {
	public void Automation(String SIM_Name, String NODE_Type, String Base_Name,
			String DEFAULT_Destination, int No_of_Nodes, boolean Arne_Required,
			boolean MOs_Required, boolean MOs_struct_defined,
			boolean send_sim_to_FTP_required,
			boolean send_sim_to_NEXUS_required, int NO_of_MOs,
			int NO_of_diff_MOs, String FTP_Address,
			String FTP_Destination_Path, String NEXUS_User_Name,
			String NEXUS_Password, String NEXUS_Version, String NEXUS_classifier)
		...
		System.out.println("\t\t\t\tNo of Nodes              :      "
				+ No_of_Nodes);
		System.out.println("\t\t\t\tArne Required            :      "
				+ Arne_Required);
		System.out.println("\t\t\t\tMOs Required             :      "
				+ MOs_Required);
		System.out.println("\t\t\t\tMOs Structure defined    :      "
				+ MOs_struct_defined);
		if (MOs_Required && !(MOs_struct_defined))
			System.out.println("\t\t\t\tNO of MOs/Node           :      "
					+ NO_of_MOs);
		if (MOs_Required && MOs_struct_defined)
			System.out.println("\t\t\t\tNO of different MOs/Node :      "
					+ NO_of_diff_MOs);

		System.out.println("\t\t\t\tsend simulation to FTP   :      "
				+ send_sim_to_FTP_required);
		if (send_sim_to_FTP_required) {
			System.out.println("\t\t\t\tFTP Address              :      "
					+ FTP_Address);
			System.out.println("\t\t\t\tFTP Destination Path     :      "
					+ FTP_Destination_Path);
		}
		System.out.println("\t\t\t\tsend_simulation to NEXUS :      "
				+ send_sim_to_NEXUS_required);
		if (send_sim_to_NEXUS_required) {
			System.out.println("\t\t\t\tNEXUS User Name          :      "
					+ NEXUS_User_Name);
			System.out.println("\t\t\t\tNEXUS Version            :      "
					+ NEXUS_Version);
			System.out.println("\t\t\t\tNEXUS classifier         :      "
					+ NEXUS_classifier);
		}
		System.out.println("");
		...
		if (send_sim_to_FTP_required) // Validating FTP Address and location
		{
			boolean FTP_value = false, Dest_value = false;
			copy_Sim_to_FTP_client copy = new copy_Sim_to_FTP_client();
			copy.ftp_client_Validate(SIM_Name + ".zip", FTP_Address,
					FTP_Destination_Path);

			File newFile_7 = new File(Logs_File_Name);
			FileWriter fw_7 = new FileWriter(newFile_7, true);
			String output_7 = exec.executeCommand("./script.sh");
			fw_7.write(output_7);
			fw_7.close();

			FTP_value = read_file.check_word_in_file(Logs_File_Name,
					"Not connected");
			Dest_value = read_file.check_word_in_file(Logs_File_Name,
					"No such file or directory");

			if (FTP_value || Dest_value) {
				del.del_sim(SIM_Name);
				mml_names.add("mml_del_sim.mml");
				call.build_script(mml_names);
				mml_names.removeAll(mml_names);
				File newFile7 = new File(Logs_File_Name);
				FileWriter fw7 = new FileWriter(newFile7, true);
				String output7 = exec.executeCommand("./script.sh");
				fw7.write(output7);
				fw7.close();

				System.out
						.println("INVALID FTP ADDRESS OR DESTINATION PATH NOT EXIST "
								+ FTP_Address);
				System.out
						.println("please correct it in the configuration file and try again");
				System.out.println(" ");
				...
				System.out.println("INFO: FTP Address Validation Completed");
		}

		if (MOs_Required && !(MOs_struct_defined)) // generating MOs with
													// Automation logic
		{
			...
			int No_of_MOs_per_Node = NO_of_MOs;
															...
															// block for M-MGw
			{
				...
				fw4.write(output4);
				fw4.close();
			} else // MOs generation block for all nodes except M-MGw
			{
				...
				// are generated
				Back_up_MOs = xml.Get_MOs(SIM_Name, No_of_MOs_per_Node,
						NE_Names[0], NODE_Type, mim_file_name, Logs_File_Name); // writing
																				// kertyle
																				// files
																				// to
																				// scale
			...
			if (!NODE_Type.toLowerCase().contains("m-mgw")) // checking the MOs
															// count generated
															// correctly or not
															// for all nodes
															// except M-MGw and
		...
		} else if (MOs_Required && MOs_struct_defined) // generating MOs
														// according to
														// structure
		{
			System.out
			...
			String Directorty = mim_file_name.substring(0,
					mim_file_name.lastIndexOf('/')); //reading mimfile directory
			// creating mp.dtd file (some xml files need mp.dtd file to get
			// parsed so creating empty file)
			check_File_in_Directory check_file = new check_File_in_Directory();
			boolean mp_avail = check_file.check_file("mp.dtd", Directorty);
			...
			if (!mp_avail) {
				check_file.create_file(Directorty + "/mp.dtd"); //creating mp.dtd
			}
			...
			String output9 = exec.executeCommand("./script.sh"); // executing
																	// the
																	// script containg mmls of sddign.createport,ipaddress
			...
			gen_MOs_def.Read_MOs(SIM_Name, NO_of_diff_MOs, mim_file_name,
					NE_Names[0] + ".mo", Logs_File_Name, NE_Names);
		...
		if (Arne_Required) {
			System.out.println("Generating Arne xml file..... ");
			generate_Arne arne = new generate_Arne();
			arne.arne_stat(SIM_Name, NE_Names, NODE_Type, No_of_Nodes);
			mml_names.add("mml_arne.mml");
		}
		...
		if (send_sim_to_FTP_required) {
			copy_Sim_to_FTP_client copy = new copy_Sim_to_FTP_client();
			copy.ftp_client(SIM_Name + ".zip", FTP_Address,
					FTP_Destination_Path);
			System.out.println("Sending Simulation to FTP ");
			File newFile7 = new File(Logs_File_Name);
			FileWriter fw7 = new FileWriter(newFile7, true);
			String output7 = exec.executeCommand("./script.sh");
			fw7.write(output7);
		...
		if (send_sim_to_NEXUS_required) {
			boolean NEXUS_value = false;
			copy_Sim_to_NEXUS nexus = new copy_Sim_to_NEXUS();
			nexus.connect_to_Nexus(SIM_Name, NEXUS_User_Name, NEXUS_Password,
					NEXUS_Version, NEXUS_classifier);
			File newFile8 = new File(Logs_File_Name);
			FileWriter fw8 = new FileWriter(newFile8, true);
			String output8 = exec.executeCommand("./script.sh");

			fw8.write(output8);
			fw8.close();

			NEXUS_value = read_file.check_word_in_file(Logs_File_Name,
			...
			if (NEXUS_value == true) {
				...
				System.out
						.println((char) 27
								+ "[31mAuthentication is failed, not able to connect to NEXUS"
								+ (char) 27 + "[30m");
				System.out
						.println("please update in the config file and try again");
				System.exit(0);
			} else
				System.out.println("Sending simulation to NEXUS ");
		}
		// deleting all mml files if created
		check_names.list_mml(NE_Names, l_max);
		System.out.println("check the logs file  \"" + Logs_File_Name
				+ "\"  for information regarding the simulation created now");
	}
}
