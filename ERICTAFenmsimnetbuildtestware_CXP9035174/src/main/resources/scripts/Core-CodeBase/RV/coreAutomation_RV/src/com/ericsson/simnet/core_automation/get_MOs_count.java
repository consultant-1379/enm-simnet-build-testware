...
public class get_MOs_count {
	...
	public ArrayList<String> divide(String SIM_Name, ArrayList<String> MO_Child, ArrayList<String> MO_Parent_String, ArrayList<Integer> No_of_available_MOs, int No_of_MOs,String Kertyle_File_Name,String Logs_File_Name) throws IOException
	{
	...
	//	System.out.println("NO_OF_MOS"+No_of_MOs);
		...
		{	
			...
			{
				if(No_of_available_MOs.get(j)>Contribution_MOs && (!MO_Child.get(j).contains("MeasurementReader")) && (!MO_Child.get(j).contains("Pm")) && (!MO_Child.get(j).contains("PM")))
				{	
			...
			if(Counter>=expected_count)
			{
				...
				MO_Parent_Line_Selected=read.Get_Parent(Kertyle_File_Name,MO_Child_Selected);
				...
				for(int n=0;n<MO_Parent_Line_Selected.size();n++)
				{
					...
					{
						MO_Parent_Line_Selected_Final.add(MO_Parent_Line_Selected.get(n));
						MO_Child_Selected_Final.add(MO_Child_Selected.get(n));
						MO_Count_Selected_Final.add(MO_Count_Selected.get(n));
						MO_Parent_String_Selected_Final.add(MO_Parent_String_Selected.get(n));
					}
				}
				...
				MOs_count=test.get_count_MOs(MO_Child_Selected_Final, MO_Parent_String_Selected_Final);
				
				while(tmp_MOs_Count>0 )
				{
					while(a<MO_Parent_Line_Selected_Final.size())
					{			
						if((MOs_count.get(a)==1) && (MO_Count_Selected_Final.get(a)>10000))
						{
							//System.out.println("entered if loop for mos check");
							Back_up_MOs.add(MO_Parent_Line_Selected_Final.get(a));
						...
						if(tmp_MOs_Count>Contribution_MOs )
						{
							...
							//to write to kertyle file for generation of MOs
							int count_MOs=(Contribution_MOs/MOs_count.get(a));
							Identity=ker.write_lines(MO_Parent_Line_Selected_Final.get(a), MO_Child_Selected_Final.get(a),count_MOs, Identity,"Kertyle1.mo",Logs_File_Name);
							tmp_MOs_Count=(tmp_MOs_Count-(count_MOs*MOs_count.get(a)));
						
						/*	System.out.println("MO Parent Selected final size of list"+MO_Parent_Line_Selected_Final.size());
							System.out.println("present iteration number "+(a+1));
							System.out.println("Remaining MOs cout to be generated "+tmp_MOs_Count);
							System.out.println("MO Parent Name Selected "+MO_Parent_Line_Selected_Final.get(a));
							System.out.println("MO Child Name Selected "+MO_Child_Selected_Final.get(a));
							System.out.println("no of mos we will get by creatin this "+MOs_count.get(a));
							System.out.println("this MOS we are creating "+count_MOs);
							*/
							a++;
						}
						...
						{
							int count_MOs=(tmp_MOs_Count/MOs_count.get(a));
							Identity=ker.write_lines(MO_Parent_Line_Selected_Final.get(a), MO_Child_Selected_Final.get(a),count_MOs, Identity,"Kertyle1.mo",Logs_File_Name);
							tmp_MOs_Count=(tmp_MOs_Count-(count_MOs*MOs_count.get(a)));
				...
				MO_Parent_Line_Selected_Final.removeAll(MO_Parent_Line_Selected_Final);
				MO_Count_Selected_Final.removeAll(MO_Count_Selected_Final);
				MO_Parent_String_Selected_Final.removeAll(MO_Parent_String_Selected_Final);
		...
		if(expected_count==1)
		{
			...
			System.out.println("May be the entered mim file name doesnot belong to the netype");
			System.exit(0);
