...
public class xml_read {
	...
	public ArrayList<String> Get_MOs(String SIM_Name, int Required_MOs, String NE_Name, String NE_Type, String xml_File_Name, String Logs_File_Name) throws IOException {
		...
		//	int Required_MOs=40000;
	        ...
	        /*    
	        Get_xml_Name xml_name=new Get_xml_Name();
	        String xml_File_Name=xml_name.get_name(NE_Type);
	        */
	        try {
		      
			//File fXmlFile = new File("/netsim/inst/zzzuserinstallation/mim_files/"+xml_File_Name);
			...
			{
				System.out.println((i+1)+" MO_Names_obsolete "+MO_Names_obsolete.get(i));
			}
		System.out.println("MO_Child.size() before "+MO_Child.size());
		System.out.println("MO_Parent_String.size() before "+MO_Parent_String.size());
		System.out.println("No_of_available_MOs.size() before "+No_of_available_MOs.size());
		*/
			
		System.out.println("INFO: No of obsolete MOs found = "+MO_Names_obsolete.size());
		if(MO_Names_obsolete.size()==0)
			System.out.println("INFO: No problem of  obsolete MOs for this NE");
		else
		System.out.println("INFO: Not populating these "+MO_Names_obsolete.size()+" obsolete MOs");
	...
	//	System.out.println("No_of_available_MOs.size() after "+No_of_available_MOs.size());
		    ...
		    //calling decision making class to decide which MO to be populated and how many times
		    get_MOs_count get=new get_MOs_count();
		/* System.out.println("MO_Parent_String size = "+MO_Parent_String.size());
		 System.out.println("MO_Child = "+MO_Child.size());
		 ...
		 System.out.println("MO_Child = "+MO_Child.get(i));
		    ...
		    Back_up_MOs=get.divide(SIM_Name,MO_Child, MO_Parent_String, No_of_available_MOs, Required_MOs,Kertyle_File_Name, Logs_File_Name);
		    return Back_up_MOs;
		}
}
