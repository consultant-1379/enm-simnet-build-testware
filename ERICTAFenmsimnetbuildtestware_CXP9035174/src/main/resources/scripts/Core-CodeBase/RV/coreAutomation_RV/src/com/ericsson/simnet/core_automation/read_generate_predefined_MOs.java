...
public class read_generate_predefined_MOs {
	...
	public void Read_MOs(String SIM_Name, int No_of_MOs, String mim_file_name,String Generated_Kertyle_File_Name, String Logs_File_Name, String[] NE_Names)
	{
		...
		try {
		    ...
		    for(int i=0;i<No_of_MOs;i++)
		    {
		    	...
		    	MO_Count.add(configfile.getProperty("Ener_No_of_MOs_of_Type"+(i+1)));
		    	MO_Count_for_each_type.add(Integer.parseInt(MO_Count.get(i)));
		    }
		    Get_MOs_xml(SIM_Name,mim_file_name, Generated_Kertyle_File_Name,Logs_File_Name,NE_Names, MO_Type, MO_Count_for_each_type); 
		} catch (IOException e) {
		    e.printStackTrace();
		}
	}
	
	public void Get_MOs_xml(String SIM_Name, String xml_File_Name, String Generated_Kertyle_File_Name, String Logs_File_Name,String[] NE_Names, ArrayList<String> MO_Type, ArrayList<Integer> MO_Count_for_each_type) throws IOException {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					if((eElement.getElementsByTagName("parent").item(0) !=null) && (eElement.getElementsByTagName("child").item(0) !=null))
					{
						if(eElement.getElementsByTagName("min").item(0) !=null)
							{
							MO_Names.add(eElement.getAttribute("name"));
							
							MOs_min.add(Integer.parseInt(eElement.getElementsByTagName("min").item(0).getTextContent()));
							if(eElement.getElementsByTagName("max").item(0) !=null)
							{ 
					            if(eElement.getElementsByTagName("max").item(0).getTextContent().length()>8)
					            MOs_max.add(80000);	
					            else
					            MOs_max.add(Integer.parseInt(eElement.getElementsByTagName("max").item(0).getTextContent()));
							}
							else
							        MOs_max.add(80000);
							}
			...
			for(int i=0;i<MO_Names.size();i++)
			{
				...
				if((MO_Names.get(i).contains("_to_")) || (MO_Names.get(i).contains("_To_")))
				{
			...
			{
				int k=0;
				//this loop is added by considering this relationship tag <relationship name="SgsnMmeTop:SystemFunctions_to_SgsnMmeSwIM:SwInventory"> 
				for(k=0;k<MO_Child.size();k++)
				{
					String[] str=MO_Child.get(k).split(":");
					String check=(str[0]+":"+MO_Type.get(m));
					
					if(MO_Child.get(k).toLowerCase().equals(check.toLowerCase()) && str[1].toLowerCase().equals(MO_Type.get(m).toLowerCase()))
					{
						...
						MO_Type_tobe_Populated.add(MO_Count_for_each_type.get(m));
						break;
					}
				}
				if(k==MO_Child.size())
				{
					...
					{
					if(MO_Child.get(l).toLowerCase().contains(MO_Type.get(m).toLowerCase()))
					{
						System.out.println("MO_Type.get(m) "+MO_Type.get(m)+ " MO_Child.get(l)"+MO_Child.get(l));
						MO_Type_Selected.add(MO_Child.get(l));
						MO_Type_Cardinality.add(No_of_available_MOs.get(l));
						MO_Type_max.add(MOs_max.get(l));
						MO_Parent_String_Selected.add(MO_Parent_String.get(l));
						MO_Type_tobe_Populated.add(MO_Count_for_each_type.get(m));
						break;	
					}
					}
					if(l==MO_Child.size())
					{
					System.out.println("WARN:cannot able to find out this MO "+MO_Type.get(m)+" in the relationship tags in the xml file");
					System.out.println("WARN:proceeding for other MOs with out populating this. populate this MO manually if required ");
					}
				}
			}
			...
			{
				//System.exit(0);
				System.out.println("you have enterd the count of MOs as \"0\". So, assuming that you want a simulation with out .mogenerator generate we are proceeding");
			}	
			else
			{
				divide_MOs_hierarchy(SIM_Name,Generated_Kertyle_File_Name, Logs_File_Name,NE_Names, MO_Type_Selected, MO_Type_Cardinality, MO_Type_tobe_Populated,MO_Type_max, MO_Parent_String_Selected);
				Del_exs_MOs del_MOs=new Del_exs_MOs();
				del_MOs.read_all_MOs(SIM_Name,NE_Names, NE_Names[0]+"_orig.mo", "Kertyle_aft_gen_6.mo", MO_Type_generated, MO_Count_generated,Logs_File_Name);
	...
	public void divide_MOs_hierarchy(String SIM_Name,String Generated_Kertyle_File_Name, String Logs_File_Name,String[] NE_Names, ArrayList<String>MO_Type_Selected, ArrayList<Integer>MO_Type_Cardinality, ArrayList<Integer>MO_Type_tobe_Populated, ArrayList<Integer> MO_Type_MAX, ArrayList<String>MO_Parent_String_Selected) throws IOException
	{
		...
		create_MOs cre_mo=new create_MOs(); //creating object for Create_MOs
		 ...
		 MO_Parent_Line_Selected=read.Get_Parent(Generated_Kertyle_File_Name,MO_Type_Selected);
		 ...
		 for(int k=0;k<MO_Parent_Line_Selected.size();k++)
		 {
			 ...
			 {
				 System.out.println("MO_Parent_Line_Selected.get(k) "+MO_Parent_Line_Selected.get(k));
				 System.out.println("MO_Type_Selected.get(k) "+MO_Type_Selected.get(k));
				 System.out.println("MO_Type_tobe_Populated.get(k) "+MO_Type_tobe_Populated.get(k));
				 System.out.println("MO_Parent_String_Selected.get(k) "+MO_Parent_String_Selected.get(k));
				 System.out.println("parent line selected is null for this mo "+MO_Type_Selected.get(k));
				 break;
			 }
			 else
		...
		String init_kertyle_file_name=NE_Names[0]+"_orig.mo";
		 {
			 MOs_available_in_gen=generate_MOs(init_kertyle_file_name, Logs_File_Name,Parent_gen_0, MO_Type_0,MO_Count_Type_0, MO_Type_Cardinality_0, MO_Type_MAX_0, MO_parent_string_0,Identity,"Kertyle_parent_def_0_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
			 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_0_", "Kertyle_aft_gen_0.mo");
			 
			 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_0_1.mo","Kertyle_parent_def_0_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=1
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_0.mo", Logs_File_Name,Parent_gen_1, MO_Type_1,MO_Count_Type_1, MO_Type_Cardinality_1, MO_Type_MAX_1, MO_parent_string_1, Identity,"Kertyle_parent_def_1_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_1_", "Kertyle_aft_gen_1.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_1_1.mo","Kertyle_parent_def_1_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=2
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_1.mo", Logs_File_Name,Parent_gen_2, MO_Type_2,MO_Count_Type_2, MO_Type_Cardinality_2, MO_Type_MAX_2, MO_parent_string_2, Identity,"Kertyle_parent_def_2_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 { 
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_2_", "Kertyle_aft_gen_2.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_2_1.mo","Kertyle_parent_def_2_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=3
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_2.mo", Logs_File_Name,Parent_gen_3, MO_Type_3,MO_Count_Type_3, MO_Type_Cardinality_3, MO_Type_MAX_3, MO_parent_string_3, Identity,"Kertyle_parent_def_3_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_3_", "Kertyle_aft_gen_3.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_3_1.mo","Kertyle_parent_def_3_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=4
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_3.mo", Logs_File_Name,Parent_gen_4, MO_Type_4,MO_Count_Type_4, MO_Type_Cardinality_4, MO_Type_MAX_4, MO_parent_string_4, Identity,"Kertyle_parent_def_4_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_4_", "Kertyle_aft_gen_4.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_4_1.mo","Kertyle_parent_def_4_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=5
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_4.mo", Logs_File_Name,Parent_gen_5, MO_Type_5,MO_Count_Type_5, MO_Type_Cardinality_5, MO_Type_MAX_5, MO_parent_string_5, Identity,"Kertyle_parent_def_5_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_5_", "Kertyle_aft_gen_5.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_5_1.mo","Kertyle_parent_def_5_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
		...
		// for parent gen_size=6
		 ...
		 {
			 MOs_available_in_gen=generate_MOs("Kertyle_aft_gen_5.mo", Logs_File_Name,Parent_gen_6, MO_Type_6,MO_Count_Type_6, MO_Type_Cardinality_6, MO_Type_MAX_6, MO_parent_string_6, Identity,"Kertyle_parent_def_6_1.mo", init_kertyle_file_name); 
			 
			 if(MOs_available_in_gen)
			 {
				 load_gen_ker.Kertyle_mml_MOs_def(SIM_Name, NE_Names, "Kertyle_parent_def_6_", "Kertyle_aft_gen_6.mo");
				
				 cre_mo.generate_file(NE_Names,"Kertyle_parent_def_6_1.mo","Kertyle_parent_def_6_"); //writing kertyle files for all nodes just by replacing node name from the initial kertyle generated
	...
	public boolean generate_MOs(String system_Generated_Kertyle_File_Name, String Logs_File_Name, ArrayList<String> MO_Parent,ArrayList<String>MO_Child,ArrayList<Integer> MO_Type_count,ArrayList<Integer> MO_Type_cardinality,ArrayList<Integer> MO_TYPE_MAX, ArrayList<String> MO_Prnt_stg, int Identity, String generated_kertyle_filename, String init_Kertyle_file)
	{
		...
		//MO_Parent_Line_latest=ker_read.Get_Parent(system_Generated_Kertyle_File_Name, MO_Child);
		//System.out.println("MO_Child.size() is "+MO_Child.size());
		
		if(MO_Child.size()==0)
		{
			//System.out.println("generating empty file");
			//Identity=ker.write_lines("","",0, 1234, generated_kertyle_filename, Logs_File_Name);
			MOs_available=false;
		}
		else
		{
			...
			if(MO_Parent.get(j).equals(null))
		   	{
		   		System.out.println("we cannnot populate this "+MO_Child.get(j));
		   		Identity=ker.write_lines("","",0, 1234, generated_kertyle_filename, Logs_File_Name);
		   	}
		   
			else
		   	{
		   		MO_Type_Count=ker_read.Get_indi_MO_Count(system_Generated_Kertyle_File_Name, MO_Child.get(j));
		   		/*
		   		if(MO_Child.get(j).contains("Sgsn_Mme:DnsServerAddress"))
		   		{
		   			System.out.println("MO_Type_count.get(j) "+MO_Type_count.get(j));
		   		...
		   		*/
		   		if(MO_Type_Count==0)
		   		{
	   				...
	   				{
			   			Identity=ker.write_lines(MO_Parent.get(j), MO_Child.get(j),MO_Type_count.get(j), 1, generated_kertyle_filename, Logs_File_Name);
		   			...
		   			{
		   				Identity=generate_MOs_splcase(system_Generated_Kertyle_File_Name, Logs_File_Name, MO_Parent.get(j), MO_Child.get(j), MO_Type_count.get(j), MO_Type_cardinality.get(j), MO_Prnt_stg.get(j),1, generated_kertyle_filename);
		   			}
		   		}
		   		...
		   		else 
		   		{
		   			...
		   			{
		   				int no_of_MOs_to_be_populated=(MO_Type_count.get(j)-MO_Type_Count);
		   				
		   				if(MO_Type_count.get(j) <= MO_TYPE_MAX.get(j))
		   				{
		   					Identity=ker.write_lines(MO_Parent.get(j), MO_Child.get(j),no_of_MOs_to_be_populated, 143, generated_kertyle_filename, Logs_File_Name);	
		   					
		   					int no_of_MOs_init=ker_read.Get_indi_MO_Count(init_Kertyle_file, MO_Child.get(j));
		   					if(no_of_MOs_init==MO_Type_Count)
		   					{
			   					MO_Type_generated.add(MO_Child.get(j));
					   			MO_Count_generated.add(no_of_MOs_to_be_populated);
			   				}
		   					else
		   					{
		   						MO_Type_generated.add(MO_Child.get(j));
					   			MO_Count_generated.add(MO_Type_count.get(j)-no_of_MOs_init);
		   					}
		   				}
		   				else
		   				{
		   				...
		   				//	System.out.println("MO_Type_cardinality.get(j) "+MO_Type_cardinality.get(j));
		   					Identity=generate_MOs_splcase(system_Generated_Kertyle_File_Name, Logs_File_Name, MO_Parent.get(j), MO_Child.get(j), MO_Type_count.get(j), MO_Type_cardinality.get(j), MO_Prnt_stg.get(j),143, generated_kertyle_filename);
		   				}
		   			}
		   			else 
		   			{
		   					   				
		   				int no_of_MOs_init=ker_read.Get_indi_MO_Count(init_Kertyle_file, MO_Child.get(j)); //initial count of MOs in file
		   				
		   					MO_Type_generated.add(MO_Child.get(j));
		   					MO_Count_generated.add((MO_Type_count.get(j))-no_of_MOs_init);
		   					
			   			//System.out.println("This MO "+MO_Child.get(j)+" is already present with count greater than or equal to requested count");
		   				//System.out.println("this MO"+MO_Child.get(j)+" present on the NE "+MO_Type_Count+" times  and req count is "+MO_Type_count.get(j)+" times"+ " So, not generating this MO");
		   				Identity=ker.write_lines(MO_Parent.get(j), MO_Child.get(j),0, 1234, generated_kertyle_filename, Logs_File_Name);	
	...
	//public int generate_MOs(String system_Generated_Kertyle_File_Name, String Logs_File_Name, ArrayList<String> MO_Parent,ArrayList<String>MO_Child,ArrayList<Integer> MO_Type_count,ArrayList<Integer> MO_Type_cardinality,ArrayList<String> MO_Prnt_stg, int Identity, String generated_kertyle_filename)
	//Identity=generate_MOs_splcase(system_Generated_Kertyle_File_Name, Logs_File_Name, MO_Parent.get(j), MO_Child.get(j), MO_Type_count.get(j), MO_Type_cardinality.get(j), MO_Prnt_stg.get(j),Identity, generated_kertyle_filename);
	public int generate_MOs_splcase(String system_Generated_Kertyle_File_Name,String Logs_File_Name, String parent, String child, int count_req, int cardinality, String parent_string, int Identity, String generated_kertyle_filename)
	{
		Kertyle_read ker_read =new Kertyle_read();
		generate_Kertyle ker=new generate_Kertyle();
		...
		ArrayList<String>MO_parent_names = new ArrayList<String>();
			...
			//
			MO_parent_names=ker_read.Get_all_MO_Names(system_Generated_Kertyle_File_Name, parent_string);
		...
		int same_MO_Count=ker_read.Get_indi_MO_Count(system_Generated_Kertyle_File_Name, parent_string);
		...
		if(((cardinality*(same_MO_Count))>= count_req))
		{
			...
			while(no_of_MOs_to_be_added>0)
			{
				...
				{
					Identity=ker.write_lines(MO_parent_Line_new.get(l), child,cardinality, Identity, generated_kertyle_filename, Logs_File_Name);
					...
					no_of_MOs_to_be_added=no_of_MOs_to_be_added-(cardinality);
				...
				else
				{
					...
					Identity=ker.write_lines(MO_parent_Line_new.get(l), child,no_of_MOs_to_be_added, Identity, generated_kertyle_filename, Logs_File_Name);
					MO_Type_generated.add(child);
		...
		{
			System.out.println("WARN:cannot populate this MO "+child+" as the cardinality for this MO is \"+MO_Type_Cardinality.get(m)+\" less than what you requested to be populated \"+MO_Type_tobe_Populated.get(m)");
			System.out.println("generating empty file");
			Identity=ker.write_lines("","",0, 1234, generated_kertyle_filename, Logs_File_Name);
	...
	public void create_run_script(String SIM_Name,int no_of_MOs, String Logs_File_Name) throws IOException
	{
	...
	private static void copy_File(String old_file, String new_file) throws IOException {
        InputStream is = null;
