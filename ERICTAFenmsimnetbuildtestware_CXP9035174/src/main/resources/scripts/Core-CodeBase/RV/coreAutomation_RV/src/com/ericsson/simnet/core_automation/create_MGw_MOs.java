...
public class create_MGw_MOs {
	...
	//public  void main(String[] args)
	{
		...
		if(No_of_MOs_Req<=1780)
		{
			...
			{
				System.out.println("Sorry we dont have support for more than 600000 MOs");
				System.out.println("Please contact Automation Team to increase the support");
			}
	}
    ...
    {
try {
PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_MOs.mo", true))); 

//creating mandatory attribute "loadModuleRef" for system must create mo which is not created "XpProgram"
	System.out.println("INFO: creating Mandatory mo \"XpProgram\" which is not created during .mogenerator generate");
		out.println("CREATE");
		out.println("(");
...
//creating mandatory attribute "bgfNetworkMoRef" for system must create mo which is not created "BgfNetworkAssociation"
		System.out.println("INFO: creating Mandatory mo \"BgfNetworkAssociation\" which is not created during .mogenerator generate");
		out.println("CREATE");
		out.println("(");
...
//creating mandatory attribute "VppEthernetInterface" for system must create mo which is not created "VppEthernetInterface"
	System.out.println("INFO: creating Mandatory mo \"VppEthernetInterface\" which is not created during .mogenerator generate");
	for (int i=0;i<28;i++)
	{
		out.println("CREATE");
		out.println("(");
		out.println("parent \"ManagedElement=1,Equipment=1,Subrack=1,Slot="+(i+1)+",PlugInUnit=1,VppUnit=1\"");
		out.println("identity 1");
...
//SwManagement_to_UpgradePackage ------30000
	/*   due to issues with mandatory attribute ftpServerIpAddress commenting this block
	for (int i=0;i<count;i++)
{
	if(diff>0)
	{
