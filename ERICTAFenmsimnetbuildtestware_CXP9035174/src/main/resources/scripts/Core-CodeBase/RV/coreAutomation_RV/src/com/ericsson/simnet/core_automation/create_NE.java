...
public class create_NE {
    ...
    public void create_ne(String simName, String neName, String neType, int numOfNes,  String nePort, String neDest)
    {
	try {
	    
	    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_cre_ne.mml", true))); 
    ...
    {
	try {
	    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_save_sim.mml", true))); 
    ...
    {
	try {
	    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_start_ne.mml", true))); 
		...
		out.println("dumpmotree:moid=\"1\",ker_out,outputfile=\""+get_addr(NE_Name+".mo")+"\";");
		//out.println(".sleep 5");
		out.println(".stop");
	    out.close();
	} catch (IOException e) {
