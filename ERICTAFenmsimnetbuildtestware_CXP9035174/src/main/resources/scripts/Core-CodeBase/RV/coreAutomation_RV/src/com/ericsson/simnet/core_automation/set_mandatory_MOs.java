...
public class set_mandatory_MOs {
	 ...
	 public boolean setMO(String SIM_Name,String NE_Type, String[] NE_Names, String Logs_File_Name) throws IndexOutOfBoundsException
	    {
		...
		if (NE_Type.toLowerCase().contains("dsc 14a-wcdma-1-ndp-1-3-1-0"))
		    {
				System.out.println("Setting attribute value for MeasurementTypeRef for node"+NE_Type);
				setattr(Filename,NE_Type, SIM_Name,NE_Names);
				//Create_Kertyle(NE_Names);
				Create_mos(SIM_Name, NE_Names);
				var= true;	
		    }
			else if((NE_Type.toLowerCase().contains("upg")) || (NE_Type.toLowerCase().contains("pgm")))
				{
					System.out.println("Setting attribute value for MeasurementTypeRef for node "+NE_Type);
					setattr(Filename,NE_Type, SIM_Name,NE_Names);
			...
			else if((NE_Type.toLowerCase().contains("dua-s")) || (NE_Type.toLowerCase().contains("dsc")) || (NE_Type.toLowerCase().contains("esapc")))
				{
					System.out.println("Setting attribute value for MeasurementTypeRef for  all nodes of type "+NE_Type);
					setattr(Filename,NE_Type, SIM_Name,NE_Names);
					var= true;
				}
			...
			else if((NE_Type.toLowerCase().contains("sbg")) && (NE_Type.toLowerCase().contains("core")))
			{
				attribute_to_set="setmoattribute:mo=\"ManagedElement="+NE_Names[0]+",SystemFunctions=1,Pm=1,PmJob=1,MeasurementReader=1\", attributes=\"measurementSpecification= [\\\"ManagedElement="+NE_Names[0]+",SystemFunctions=1,Pm=1,PmJob=1\\\", \\\"\\\"]\";";	
				set_attr_mml(SIM_Name,NE_Names,attribute_to_set);
				var=true;
			}
			else if((NE_Type.toLowerCase().contains("sgsn")) && (NE_Type.toLowerCase().contains("wpp")))
			{
				attribute_to_set="setmoattribute:mo=\"ManagedElement="+NE_Names[0]+",SystemFunctions=1,Pm=1,PmJob=1,MeasurementReader=1\", attributes=\"measurementSpecification= [\\\"ManagedElement="+NE_Names[0]+",SystemFunctions=1,Pm=1,PmJob=1\\\", \\\"\\\"]\";";	
				set_attr_mml(SIM_Name,NE_Names,attribute_to_set);
				var=true;
			...
			else if(NE_Type.toLowerCase().contains("sdnc-p"))
			{
				for(int i=0;i<NE_Names.length;i++)
				{
					...
					MO="CmwPm:PmGroup";
					kert.write_lines(Parent, MO, 1, 1, "set_mos"+(i+1)+".mo", Logs_File_Name); //creates mandatory mo CmwPm:PmGroup
					...
					kert.write_lines(Parent, MO, 1, 2, "set_mos"+(i+1)+".mo", Logs_File_Name); //creates mandatory mo CmwPm:MeasurementType
					...
					kert.set_mos_kertyle(mo, attributes_set, "set_mos"+(i+1)+".mo", Logs_File_Name);
					attributes_set.removeAll(attributes_set); //sets the attributes to null
			...
			else if(NE_Type.toLowerCase().contains("esasn"))
			{
				...
				attributes_set.add("\"dnPrefix\" String \"SubNetwork=ONRM_ROOT_MO_R,SubNetwork=IMS,MeContext="+NE_Names[0]+"\"");
				for(int i=0;i<NE_Names.length;i++)
				{
				kert.set_mos_kertyle(mo, attributes_set, "set_mos"+(i+1)+".mo", Logs_File_Name);
				}
	    ...
	    public void setattr(String FileName, String NE_Type, String SIM_Name, String[] NE_Names) throws IndexOutOfBoundsException
	    {
		try {
		    ...
		    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_set_mos.mml", true))); 
		    ...
		    out.println(".start -parallel");
			    ...
			    for(int i=0;i<NE_Names.length;i++)
				{
					...
					{	
					    ...
					    String First_Attr = get_MO_Parent.get(k).replaceAll(Get_NE_Name, NE_Names[i]);
					    String First_String = First_Attr.substring(First_Attr.indexOf("\"")+1,First_Attr.indexOf(","));
					    String Actual_Value = "[\\\"\\\",\\\""+First_String+"\\\"]";
					    String Actual_Parent = get_MO_Parent.get(k).replaceAll("parent","").replaceAll("ComTop:","").replaceAll("ECIM_PM:","").replaceAll("CmwPm:","").replaceAll(Get_NE_Name, NE_Names[i]);

					    out.println("setmoattribute:mo="+Actual_Parent+",attributes=\"measurementSpecification=" +Actual_Value+"\";");
					}
				}	
			    out.close();
			    BR1.close();
	    ...
	    {
		try {
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_create_mos.mml", true)));
		out.println(".open "+SIM_Name);
		out.println(".select network");
		out.println(".start -parallel");
		for(int i=0;i<NE_Names.length-1;i++)
			...
			out.println(".select "+NE_Names[i]);
			out.println("kertayle:file=\""+get_addr("mml_create_mos"+NE_Names[i]+".mo")+"\";");
			//	out.println(".stop");
	    }
		out.close();	  
	}
	...
	public void Create_Kertyle(String NE_Names)
	    {
		try{
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_create_mos"+NE_Names+".mo", true)));
		out.println("CREATE");	
		out.println("(");
	...
	public void set_mos_upg_pgm(String SIM_Name,String[] NE_Names)
		...
		for(int i=0;i<NE_Names.length;i++)
			...
			out.println(".select "+NE_Names[i]);
			out.println("kertayle:file=\""+get_addr("mml_set_name_mos_"+NE_Names[i]+".mo")+"\";");
			//	out.println(".stop");
	    }
		out.close();	  
	}
	...
	public void Create_Kertyle_name(String NE_Names)
	    {
		try{
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_set_name_mos_"+NE_Names+".mo", true)));
		out.println("SET");
		out.println("(");
	...
	public void set_attr_mml(String SIM_Name, String[] NE_Names, String attribute_to_set )
	{
		try
		{
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("mml_set_mos.mml", true)));
		...
		for(int i=0;i<(NE_Names.length);i++)
		{
			attribute=attribute_to_set.replaceAll(NE_Names[0],NE_Names[i]);
			out.println(".select "+NE_Names[i]);
			out.println(".start");
	...
	public void create_mos_mml(String SIM_Name,String[] NE_Names)
	{
		...
		for(int i=0;i<(NE_Names.length);i++)
		{
			out.println(".select "+NE_Names[i]);
			out.println(".start");
			out.println("kertayle:file=\""+get_addr("set_mos"+(i+1)+".mo")+"\";");
			out.println(".stop");
