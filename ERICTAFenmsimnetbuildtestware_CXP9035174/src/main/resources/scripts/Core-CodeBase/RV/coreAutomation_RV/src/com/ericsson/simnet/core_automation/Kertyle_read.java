...
public class Kertyle_read {
	...
	public ArrayList<String> Get_Parent(String File_Name, ArrayList<String> MO_Child)
	   {
	 ...
	 //System.out.println(MO_Child); 
		for(i=0;i<MO_Child.size();i++) 
		{
			...
			if(j==sa1)
			{
				...
				System.out.println("cannot able to find parent line of this MOType "+MO_Child.get(i));
				MO_Parent.add(null);
			}
		}
	   }
	   catch(Exception e){
     System.out.println("Error while reading file line by line:Kertyle_Read" 
     + e.getMessage()); 
	   }
	return MO_Parent;
	...
	public int Get_indi_MO_Count(String File_Name, String MO_Type)
	{
		...
		try{
			...
			sa1=a1.size();
		     ...
		     System.out.println("Error while reading file line by line:Kertyle_Read_fun_2" 
		     + e.getMessage()); 
			   }
		return Count;
	}
		...
		try{
			...
			sa1=a1.size();
				...
				int j=0;
					...
					if((j==sa1) && (Count==0))
					{
						System.out.println("This "+MO_Type+" is not found in the recently generated kertyle file");
						//System.out.println("count = "+Count);
					}
