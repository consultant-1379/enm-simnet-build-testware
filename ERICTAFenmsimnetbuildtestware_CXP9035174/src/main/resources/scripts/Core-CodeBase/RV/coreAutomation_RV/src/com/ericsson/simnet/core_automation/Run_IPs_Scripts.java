	...
	public void check_directory()
	{
		...
		if (!theDir.exists()) {
		    ...
		    try{
		     ...
		     if(result) {  
		    	 if (i==0)
		       System.out.println("INFO: "+direc[i]+" Directory created to store free IPs files generated ");  
		    	 else 
		    		 System.out.println("INFO: "+direc[i]+" Directory created to store LOG files generated "); 	 
		     }
		  }
		  else
		  {
			...
			if(f1.exists() && f2.exists() && f3.exists())
				...
				{
				System.out.println("INFO: All Scripts required are not available, we are creating them");
