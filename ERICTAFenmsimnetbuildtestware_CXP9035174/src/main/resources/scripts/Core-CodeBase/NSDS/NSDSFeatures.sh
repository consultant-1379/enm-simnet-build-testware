...
# Purpose     : Loads features for vNSDS nodes based on WCG node type
...
echo "Example: $0 CORE-ST-vNSDS-2.4.0-V1x2"

}
neType(){

echo "ERROR: The script runs only for vNSDS nodes"
}
########################################################################
...
Path=`pwd`
########################################################################
#To check the if simname is NSDS#
########################################################################
if [[ $simName != *"NSDS"* ]]
then
neType
...
/netsim/inst/netsim_pipe < nsds.mml
