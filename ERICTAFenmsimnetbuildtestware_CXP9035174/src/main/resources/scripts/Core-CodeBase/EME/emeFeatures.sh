...
# Purpose     : Loads features for EME nodes based on node type
...
echo "Example: $0 CORE-ST-vEME-16A-V4x5 "

}
neType(){

echo "ERROR: The script runs only for EME nodes"
}
########################################################################
...
Path=`pwd`
########################################################################
#To check the if simname is EME#
########################################################################
if [[ $simName != *"vEME"* ]]
then
neType
...
(
    ...
    exception none
    nrOfAttributes 1
    "fileLocation" String "/storage/no-backup/com-apr9010443/PerformanceManagementReportFiles"
)
SET
(
...
SET
(
    mo "ManagedElement=$nodeName,SystemFunctions=1,Pm=1,PmGroup=SdsCacheCounterGroup"
    exception none
    nrOfAttributes 1
    "moClass" Struct
        nrOfElements 4
        "moClassName" String "SdsCacheCounterGroup"
        "mimName" String "SDS_PM"
        "mimVersion" String "0"
        "mimRelease" String "1"
...
/netsim/inst/netsim_pipe < eme.mml
