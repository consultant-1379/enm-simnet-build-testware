package com.ericsson.oss.ci.simnet.test.operators;
...
import com.ericsson.oss.ci.simnet.test.utils.HostHandler;
import com.ericsson.oss.ci.simnet.test.utils.TafFileUtils;
import com.ericsson.oss.ci.simnet.test.utils.ZipUtility;
...
public class CLIScriptExecutionOperator implements ScriptExecutionOperator {
    ...
    private static final String REMOTE_FOLDER_TAF_SCRIPTS_LOCATION = "/netsim/Core_Build/ERICTAFenmsimnetbuildtestware_CXP9035174/src/main/resources/";
    ...
    private static final String JENKINS_ENM_NI_SIMNET_JAR_FILE_SEARCH_TERM = "lib.*CXP9035174";

    /** Local zip file name for zipped embedded scripts */
    private static final String ZIP_FILE_NAME = "enm-simnet-build-testware.zip";
    ...
    private static boolean setUpRemoteServerFolder(final CLICommandHelper cliComandHelper) {
        ...
        cliComandHelper.execute("mkdir -v -p /netsim/Core_Build/ERICTAFenmsimnetbuildtestware_CXP9035174/src/main/resources; chmod -R 777 /netsim/Core_Build/; chown -v netsim:netsim /netsim/Core_Build/ERICTAFenmsimnetbuildtestware_CXP9035174/src/main/resources");
    ...
    private static boolean copyScriptsFolderToRemoteServer(final CLICommandHelper cliComandHelper) {
        ...
        try {
            ...
            if (Pattern.compile("testware").matcher(currentPath).find()) {
                ...
                final String destUnzipFolderName = "enm-simnet-build-testware";
	...
	String nodeType = convertToPerlCmdArg(DataHandler.getAttribute("node_type").toString());
	String network_size = convertToPerlCmdArg(DataHandler.getAttribute("network_size").toString());
        String nssDrop = convertToPerlCmdArg(DataHandler.getAttribute("nssDrop").toString());
	String nodeTemplate = convertToPerlCmdArg(DataHandler.getAttribute("node_template").toString());
	String patchList = convertToPerlCmdArg(DataHandler.getAttribute("patchList").toString());
        ...
        try {
            ...
            if (command.contains("simnet") && command.contains("triggerBuild.sh")) {
                String tmpCommand = command;
                ...
                simnetParam = " " + simName + " " + nodeType + " " + network_size + " " + nssDrop + " " + nodeTemplate + " " + patchList;
